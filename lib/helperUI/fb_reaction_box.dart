...
class FbReactionState extends State<FbReaction> with TickerProviderStateMixin {
          ...
          Container(
            child: Stack(
              children: <Widget>[
                ...
                renderBtnLike(postId: widget.postId),
                whichIconUserChoose == 1 && !isDragging
                    ? Container(
                        child: Transform.scale(
                          child: Image.asset(
                            'assets/reaction_emoji/like.gif',
                            width: 40.0,
                            height: 40.0,
                          ),
                ...
                whichIconUserChoose == 2 && !isDragging
                    ? Container(
                        child: Transform.scale(
                          child: Image.asset(
                            'assets/reaction_emoji/love.gif',
                            width: 40.0,
                            height: 40.0,
                          ),
                          scale: this.zoomIconWhenRelease.value,
                ...
                whichIconUserChoose == 3 && !isDragging
                    ? Container(
                        child: Transform.scale(
                          child: Image.asset(
                            'assets/reaction_emoji/haha.gif',
                            width: 40.0,
                            height: 40.0,
                          ),
                          scale: this.zoomIconWhenRelease.value,
                ...
                whichIconUserChoose == 6 && !isDragging
                    ? Container(
                        child: Transform.scale(
                          child: Image.asset(
                            'assets/reaction_emoji/angry.gif',
                            width: 40.0,
                            height: 40.0,
                          ),
                          scale: this.zoomIconWhenRelease.value,
  ...
  Widget renderIcons() {
    return Container(
      ...
      width: 300.0,
      height: 100.0,
      margin: EdgeInsets.only(left: this.moveRightGroupIcon.value, top: 10.0),
      // uncomment here to see area of draggable
      // color: Colors.amber.withOpacity(0.5),
    );
  }

  Widget renderBtnLike({String postId}) {
    return Container(
      child: GestureDetector(
        onTapDown: onTapDownBtn,
        onTapUp: onTapUpBtn,
        onTap: () {
          onTapBtn(postId: postId);
        },
        child: Container(
          child: Transform.scale(
  ...
  String getImageIconBtn() {
    if (!isLongPress && isLiked) {
      return 'assets/reaction_emoji/ic_like_fill.png';
    } else if (!isDragging) {
      switch (whichIconUserChoose) {
        case 1:
          return 'assets/reaction_emoji/ic_like_fill.png';
        case 2:
          return 'assets/reaction_emoji/love2.png';
        case 3:
          return 'assets/reaction_emoji/haha2.png';
        case 4:
          return 'assets/reaction_emoji/wow2.png';
        case 5:
          return 'assets/reaction_emoji/sad2.png';
        case 6:
          return 'assets/reaction_emoji/angry2.png';
        default:
          return 'assets/reaction_emoji/ic_like.png';
      }
    } else {
      return 'assets/reaction_emoji/ic_like.png';
    }
  ...
  void onHorizontalDragUpdateBoxIcon(DragUpdateDetails dragUpdateDetail) {
    ...
    if (dragUpdateDetail.globalPosition.dy >= 10 &&
        dragUpdateDetail.globalPosition.dy <= 100) {
      isDragging = true;
      isDraggingOutside = false;
  ...
  void handleWhenDragBetweenIcon(int currentIcon) {
    playSound('assets/sounds/icon_focus.mp3');
  ...
  void onTapBtn({String postId}) {
    if (!isLongPress) {
      if (whichIconUserChoose == 0) {
        isLiked = !isLiked;
        print('liked');
        isLiked
            ? DatabaseHandler().like(postId)
            : DatabaseHandler().disLike(postId);
      } else {
        whichIconUserChoose = 0;
      }
      if (isLiked) {
